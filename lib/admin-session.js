...
import { serviceHeaders, timingSafeEqualStr, fetchJsonWithTimeout } from './security.js';
...
export async function createAdminSession() {
  ...
  const pack = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/admin_sessions`,
    {
      method: 'POST',
  ...
  if (pack.timedOut) {
    return { ok: false, error: 'session_create_timeout' };
  }
  if (!pack.ok) {
    console.error('admin_sessions insert failed:', pack.status);
...
export async function requireAdminSession(req) {
  ...
  if (pack.timedOut || pack.error === 'network') {
    return { ok: false, status: 503, error: 'session_lookup_timeout' };
  }
  const rows = pack.data;
  if (!pack.ok || !Array.isArray(rows) || !rows[0]) {
