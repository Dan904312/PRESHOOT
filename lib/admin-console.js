...
export async function fetchSetting(key) {
  ...
  if (pack.timedOut || !pack.ok || !Array.isArray(pack.data) || !pack.data[0]) return null;
  return pack.data[0].value || null;
}
...
export async function fetchUsageRollup(sinceIso) {
  ...
  const pack = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/rpc/admin_usage_rollup`,
    {
      method: 'POST',
      headers: serviceHeaders(),
  ...
  if (pack.timedOut) return [];
  if (pack.ok && Array.isArray(pack.data)) return pack.data;
...
async function fetchUsageRollupFromTable(sinceIso) {
  ...
  const pack = await fetchJsonWithTimeout(url, { headers: serviceHeaders() }, 4500);
  if (pack.timedOut || !pack.ok || !Array.isArray(pack.data)) return [];
  return pack.data.map((row) => ({
    user_id: row.user_id,
...
export async function fetchUserUsageTotals(userId) {
  const SUPA_URL = process.env.SUPABASE_URL;
  if (!SUPA_URL || !userId) {
    return { scans: 0, director: 0, research: 0, ai: 0, cost: 0 };
  }
  const pack = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/usage_events?user_id=eq.${encodeURIComponent(userId)}&select=event_type,estimated_cost&status=eq.success&limit=4000`,
    { headers: serviceHeaders() },
    4500
  );
  if (pack.timedOut || !pack.ok || !Array.isArray(pack.data)) {
    return { scans: 0, director: 0, research: 0, ai: 0, cost: 0 };
  }
...
export async function probeUsageLedger() {
  ...
  if (pack.timedOut) return { ok: false, error: 'timeout' };
  const body = pack.data;
  if (pack.status === 404 || (body && (body.code === 'PGRST205' || /does not exist/i.test(String(body.message || ''))))) {
    return { ok: false, error: 'missing_table' };
  }
  if (!pack.ok) {
    return { ok: false, error: 'query_failed', status: pack.status };
  }
  return {
    ok: true,
    total_events: Array.isArray(body) ? body.length : 0
  };
}
...
export async function fetchDailyUsageRpc(sinceIso) {
  ...
  if (!SUPA_URL) return null;
  const pack = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/rpc/admin_daily_usage`,
    {
      method: 'POST',
      headers: serviceHeaders(),
...
export async function countProductErrors(sinceIso) {
  ...
  const rows = pack.ok && Array.isArray(pack.data) ? pack.data : [];
  let rateLimited = 0;
  let server = 0;
  const byEp = {};
  rows.forEach((e) => {
...
export async function fetchAuthAudit(limit) {
  ...
  if (!pack.ok) return { available: false, entries: [], status: pack.status };
  ...
  return {
    ...
    entries: rows.slice(0, 40).map((e) => ({
      ...
      actor_id: e.payload && e.payload.actor_id ? String(e.payload.actor_id).slice(0, 80) : null
    }))
  };
}
...
export async function probeSystem() {
  ...
  const db = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/users?select=user_id&limit=1`,
    { headers: serviceHeaders() },
    3500
  );
  out.database = db.timedOut
    ? { state: 'unavailable', detail: 'Timed out' }
    : db.ok
      ? { state: 'operational', detail: 'PostgREST reachable' }
      : { state: 'degraded', detail: 'HTTP ' + db.status };

  const auth = await fetchJsonWithTimeout(
    `${SUPA_URL}/auth/v1/health`,
    { headers: { apikey: key } },
    3500
  );
  out.auth = auth.timedOut
    ? { state: 'unavailable', detail: 'Timed out' }
    : auth.ok
      ? { state: 'operational', detail: 'GoTrue health OK' }
      : { state: 'degraded', detail: 'HTTP ' + auth.status };

  const storage = await fetchJsonWithTimeout(
    `${SUPA_URL}/storage/v1/bucket`,
    { headers: { apikey: key, Authorization: 'Bearer ' + key } },
    3500
  );
  out.storage = storage.timedOut
    ? { state: 'unavailable', detail: 'Timed out' }
    : storage.ok
      ? { state: 'operational', detail: 'Storage API reachable' }
      : { state: 'degraded', detail: 'HTTP ' + storage.status };
