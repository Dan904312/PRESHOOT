...
export const JOIN_CODE_DEFAULT_ROLE = 'editor';
export const JOIN_CODE_ROLES = ['editor', 'commenter', 'viewer'];
...
export function isJoinCode(raw) {
  return JOIN_CODE_RE.test(normalizeJoinCode(raw));
}

export function joinCodePepper() {
  return process.env.JOIN_CODE_PEPPER || process.env.SUPABASE_SERVICE_KEY || 'preshoot-join-v1';
}

/** HMAC so 6-digit codes cannot be rainbow-tabled from the stored hash. */
export function hashJoinCode(code, pepper) {
  const p = pepper == null || pepper === '' ? joinCodePepper() : String(pepper);
  return crypto
    .createHmac('sha256', p)
    .update('join-code:' + normalizeJoinCode(code), 'utf8')
    .digest('hex');
}
...
function missingJoinCodeColumn(data) {
  const blob = typeof data === 'string' ? data : JSON.stringify(data || {});
  return /join_code_hash|join_code_role|42703|schema cache/i.test(blob);
}
...
export async function createSharedWorkspace(userId, { name, slug } = {}) {
  const cfg = restBase();
  if (!cfg) {
    console.error('workspace_create_config_missing', {
  ...
  if (!createWs.ok) {
    console.error('workspace_create_database_error', {
      step: 'workspaces',
      status: createWs.status,
      code: createWs.data && createWs.data.code
    });
    return {
      ok: false,
...
function publicJoinCodeStatus(row) {
  return {
    has_code: !!(row && row.join_code_hash),
    role: (row && JOIN_CODE_ROLES.includes(row.join_code_role)
      ? row.join_code_role
      : JOIN_CODE_DEFAULT_ROLE),
    generated_at: (row && row.join_code_generated_at) || null
  };
}

export async function getJoinCodeStatus(actorId, workspaceId) {
  const m = await assertWorkspaceRole(actorId, workspaceId, ['owner']);
  if (!m.ok) return m;
  if (m.workspace.kind === 'personal') {
    return { ok: false, status: 403, error: 'personal_workspace_private' };
  ...
  const rows = await restJson(
    'GET',
    `workspaces?id=eq.${encodeURIComponent(workspaceId)}&select=join_code_hash,join_code_role,join_code_generated_at&limit=1`
  );
  if (!rows.ok) {
    if (missingJoinCodeColumn(rows.data)) {
      return {
        ok: false,
        status: 500,
        error: 'join_codes_unavailable',
        message: 'Join codes are not enabled on this database yet.'
      };
    }
    return { ok: false, status: 500, error: 'join_code_lookup_failed' };
  }
  const row = Array.isArray(rows.data) ? rows.data[0] : null;
  return { ok: true, join_code: publicJoinCodeStatus(row) };
}

export async function regenerateJoinCode(actorId, workspaceId, { role } = {}) {
  const m = await assertWorkspaceRole(actorId, workspaceId, ['owner']);
  if (!m.ok) return m;
  if (m.workspace.kind === 'personal') {
    return { ok: false, status: 403, error: 'personal_workspace_private' };
  ...
  if (m.workspace.archived_at) {
    return { ok: false, status: 409, error: 'workspace_archived' };
  }
  const nextRole = JOIN_CODE_ROLES.includes(role) ? role : JOIN_CODE_DEFAULT_ROLE;
  const cfg = restBase();
  if (!cfg) return { ok: false, status: 500, error: 'no_config' };
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const r = await fetch(
      `${cfg.url}/rest/v1/workspaces?id=eq.${encodeURIComponent(workspaceId)}`,
      {
        ...
        body: JSON.stringify({
          join_code_hash: hash,
          join_code_role: nextRole,
          join_code_generated_at: new Date().toISOString(),
          join_code_generated_by: actorId,
          updated_at: new Date().toISOString()
        })
      }
    );
    ...
    if (r.ok && Array.isArray(rows) && rows[0]) {
      return {
        ok: true,
        code,
        join_code: publicJoinCodeStatus(rows[0])
      };
    }
    lastError = rows;
    const blob = JSON.stringify(rows || {});
    if (missingJoinCodeColumn(rows)) {
      return {
        ok: false,
        status: 500,
        error: 'join_codes_unavailable',
    ...
    /* Unique hash collision — retry with a new code. */
    if (r.status === 409 || /unique|duplicate/i.test(blob)) continue;
    return { ok: false, status: 500, error: 'join_code_rotate_failed', detail: lastError };
  }
  return { ok: false, status: 500, error: 'join_code_rotate_failed', detail: lastError };
}

export async function joinWorkspaceByCode(userId, rawCode) {
  ...
  if (isUuid(rawCode)) {
    return {
      ...
      message: 'Enter the 6-digit join code, not a workspace id.'
    };
  }
  ...
  if (!isJoinCode(code)) {
    return {
      ...
      status: 400,
      error: 'invalid_join_code',
      message: 'Enter a 6-digit join code.'
    };
  }
  ...
  const found = await restJson(
    'GET',
    `workspaces?join_code_hash=eq.${encodeURIComponent(hash)}&select=id,name,slug,owner_id,kind,archived_at,join_code_role&limit=1`
  );
  if (!found.ok) {
    if (missingJoinCodeColumn(found.data)) {
      return {
        ok: false,
        status: 500,
        error: 'join_codes_unavailable',
    ...
    return { ok: false, status: 500, error: 'join_lookup_failed' };
  }
  const workspace = Array.isArray(found.data) ? found.data[0] : null;
  if (!workspace) {
    return {
      ok: false,
      status: 404,
      error: 'join_code_invalid',
      message: 'That join code is not valid.'
    };
  }
  if (workspace.kind !== 'shared' || workspace.archived_at) {
    return {
      ok: false,
      status: 409,
      error: 'workspace_archived',
      message: 'This workspace is no longer accepting joins.'
    };
  }

  const existing = await restJson(
    'GET',
    `workspace_members?workspace_id=eq.${encodeURIComponent(workspace.id)}&user_id=eq.${encodeURIComponent(userId)}&select=role&limit=1`
  );
  const member = Array.isArray(existing.data) ? existing.data[0] : null;
  if (member) {
    return {
      ok: true,
      already_member: true,
  ...
  const joinRole = JOIN_CODE_ROLES.includes(workspace.join_code_role)
    ? workspace.join_code_role
    : JOIN_CODE_DEFAULT_ROLE;
  const cfg = restBase();
  if (!cfg) return { ok: false, status: 500, error: 'no_config' };
  const r = await fetch(`${cfg.url}/rest/v1/workspace_members`, {
  ...
  const rows = await r.json().catch(() => null);
  if (!r.ok) {
    return { ok: false, status: 500, error: 'join_failed', detail: rows };
  }
  return {
    ok: true,
