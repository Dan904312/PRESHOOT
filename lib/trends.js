...
export async function fetchYouTubeSearch(query, region, apiKey) {
  ...
  const r = await fetchJson('https://www.googleapis.com/youtube/v3/search?' + params.toString(), 8000);
  if (!r.ok || !r.json) {
    return {
      items: [],
 ...
 */
async function fetchRegionParts(region, youtubeKey) {
...
export async function getTrendDataset(opts) {
  ...
  try {
    if (region === 'GLOBAL') {
      const extra = await Promise.all([
        fetchRegionParts('US', youtubeKey),
      ...
      parts = {
        google: {
          items: []
            ...
            .concat(extra[1] && extra[1].items)
            .concat(extra[2] && extra[2].items)
            .concat(extra[3] && extra[3].items)
            .filter(Boolean),
      ...
      parts = await fetchRegionParts(region, youtubeKey);
    }
  } catch (e) {
  ...
  const google = parts.google;
  const youtube = parts.youtube;
  const apple = parts.apple;
  const tiktok = parts.tiktok;
  const youtubeCharts = parts.youtubeCharts;
