...
export async function writeAdminNotification(entry) {
  ...
  const pack = await fetchJsonWithTimeout(
    `${SUPA_URL}/rest/v1/admin_notifications?on_conflict=source_ref`,
    {
      method: 'POST',
  ...
  if (pack.status === 409) return { ok: true, duplicate: true };
  if (pack.timedOut || !pack.ok) return { ok: false, error: pack.timedOut ? 'timeout' : 'write_failed', status: pack.status };
  return { ok: true, row: Array.isArray(pack.data) ? pack.data[0] : pack.data };
}
...
export async function listAdminNotifications({ unreadOnly, limit }) {
  ...
  const [listPack, countPack] = await Promise.all([
    fetchJsonWithTimeout(url, { headers: serviceHeaders() }, 4000),
  ...
  if (listPack.timedOut) {
    return { ok: false, error: 'timeout', notifications: [], unread: 0 };
  }
  if (!listPack.ok) {
    return { ok: false, error: 'list_failed', notifications: [], unread: 0, status: listPack.status };
  }
  const rows = Array.isArray(listPack.data) ? listPack.data : [];
  const contentRange =
    (countPack.headers && countPack.headers.get && countPack.headers.get('content-range')) || '';
...
export async function markNotificationsRead({ ids, all }) {
  ...
  return { ok: pack.ok, error: pack.ok ? null : pack.timedOut ? 'timeout' : 'mark_failed' };
}
