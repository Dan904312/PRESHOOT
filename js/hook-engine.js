...
(function (global) {
  ...
  function buildScanPromptSection(ctx) {
    ...
    return [
      ...
      '- hook: Hook 1 (word-for-word spoken / on-screen first line, 3 seconds or less when possible)',
      '- altHooks: Hook 2, Hook 3, Hook 4 (different frameworks or angles, same subject). All four are equal candidates.',
      '- hookFramework: framework id used for Hook 1',
  ...
  function buildDirectorPromptSection() {
    return [
      ...
      '- Always open with a strong hook. When proposing openings, offer 4 equal hook candidates (Hook 1-4).',
      '- Build the ENTIRE script around the selected hook(s): setup → payoff must match the opening promise.',
